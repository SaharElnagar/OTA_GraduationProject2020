...
//check if transmission mailbox empty
	if (CAN2->TSR & (1 << (26 + MailBox))) {
		...
		//set CAN identifier
		if (msg.IDE)
			CAN2->sTxMailBox[MailBox].TIR |= (msg.id << 3);   /*TODO 2 later*/
		else
			CAN2->sTxMailBox[MailBox].TIR |= (msg.id << 21);
		//set data length
		CAN2->sTxMailBox[MailBox].TDTR = (CAN2->sTxMailBox[MailBox].TDTR
				& (~0x0f)) | msg.data_legth;
		//reset transmit data registers
		...
		for (int i = 0; i < msg.data_legth; i++) {
			if (i < 4)
				CAN2->sTxMailBox[MailBox].TDLR |= (msg.pdata[i] << (8 * i));
			else
...
enum_error_type CAN2_receive(CAN_message_TypeDef* msg, uint8_t* match_index) {
	...
	if (CAN2->RF0R & CAN_RF0R_FMP0) {
		*match_index = (CAN2->sFIFOMailBox[0].RDTR & CAN_RDT0R_FMI) >> 8;
		...
		if (msg->IDE)
			msg->id = (CAN2->sFIFOMailBox[0].RIR
					& (CAN_RI0R_EXID | CAN_RI0R_STID)) >> 3;          /*TODO 5*/
		else
			msg->id = (CAN2->sFIFOMailBox[0].RIR & CAN_RI0R_STID) >> 21;
		for (int i = 0; i < msg->data_legth; i++) {
			if (i < 4)
				msg->pdata[i] = (CAN2->sFIFOMailBox[0].RDLR >> (i * 8)) & 0xFF;
			else
				msg->pdata[i] = (CAN2->sFIFOMailBox[0].RDHR >> ((i - 4) * 8))
						& 0xFF;
		}
		...
		*match_index = (CAN2->sFIFOMailBox[1].RDTR & CAN_RDT1R_FMI) >> 8;
		...
		if (msg->IDE)
			msg->id = (CAN2->sFIFOMailBox[1].RIR
					& (CAN_RI1R_EXID | CAN_RI1R_STID)) >> 3;
		else
			msg->id = (CAN2->sFIFOMailBox[1].RIR & CAN_RI1R_STID) >> 21;
		for (int i = 0; i < msg->data_legth; i++) {
			if (i < 4)
				msg->pdata[i] = (CAN2->sFIFOMailBox[1].RDLR >> (i * 8)) & 0xFF;
			else
				msg->pdata[i] = (CAN2->sFIFOMailBox[1].RDHR >> ((i - 4) * 8))
						& 0xFF;
		}
